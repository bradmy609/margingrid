...
og_df = cleanDF(pd.read_sql("SELECT {} FROM PolygonVW LIMIT 100000, 300000".format(temp_columns), con=engine).drop('index', axis=1).rename({'ts': 'index'}, axis=1).astype('float'))
